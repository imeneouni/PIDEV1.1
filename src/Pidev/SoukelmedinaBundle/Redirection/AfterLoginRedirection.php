...
namespace Pidev\SoukelmedinaBundle\Redirection;
...
class AfterLoginRedirection implements AuthenticationSuccessHandlerInterface
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token)
    {
        ...
        if (in_array('ROLE_ADMIN', $rolesTab, true) )
            $redirection = new RedirectResponse($this->router->generate('testback'));
        elseif (in_array('ROLE_VENDEUR', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('testP'));
